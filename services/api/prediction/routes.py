...
async def predict(request: SinglePredictRequest):
    ...
    result = (
        await state.orchestrator.predict_single(  # 通过orchestrator调用单个预测功能
            PredictionRequest(
...
async def predict_batch(request: BatchPredictRequest):
    ...
    requests = [
        PredictionRequest(sequence=req.sequence, peptide_id=req.peptide_id)
        for req in request.sequences
    ]

    results = (
        await state.orchestrator.predict_batch(  # 通过orchestrator调用批量预测功能
            requests, tools=request.tools
        )
    )
