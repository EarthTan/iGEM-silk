...
环境变量:
    ...
    OMEGAFOLD_WEIGHTS_URL  权重下载 URL 覆盖（镜像源时使用）
...
class OmegaFoldService(StructureService):
    ...
    async def load_model(self) -> None:
        ...
        default_urls = {
            1: "https://helixon.s3.amazonaws.com/release1.pt",
            2: "https://helixon.s3.amazonaws.com/release2.pt",
        }
        env_url = os.environ.get("OMEGAFOLD_WEIGHTS_URL")
        if env_url:
            weights_url = env_url
        else:
            weights_url = default_urls[self._model_idx]
