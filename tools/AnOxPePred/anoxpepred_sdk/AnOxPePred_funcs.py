...
def create_AnOxPePred_v1(hps):
    ...
    class AnOxPePred_v1(Model):
        def __init__(self, hps):
            ...
            self.conv = Conv1D(filters=128, kernel_size=3, strides=1, activation='elu', padding='same', kernel_initializer=tf.keras.initializers.glorot_normal(seed=1))
            self.maxpool = AveragePooling1D(pool_size=3, strides=3)
            self.dropout1 = Dropout(0.1)

            self.flatten = Flatten()
            self.d1 = Dense(256, activation='elu', kernel_initializer=tf.keras.initializers.glorot_normal(seed=1))
...
def focal_loss(gamma=3, alpha=.25, mask_val=-1):
    """
...
def output_df(x_data, y_label, y_pred, tag):

    df_pred = pd.DataFrame(y_pred, columns=['Pred_FRS','Pred_Chel'])
    df_label = pd.DataFrame(y_label,  columns=['FRS','Chelator'])
    df_length = pd.DataFrame([int(i.sum()) for i in x_data], columns=['Length'])
    df_length['tag'] = tag

    return pd.concat([df_pred, df_label, df_length], axis=1)
...
def length_metrics(a_df, new_bins, new_names):

    def this_guy(a_length, new_bins):
        for num, val in enumerate(new_bins):
            if a_length in val:
                return num
    ...
    tmp_df['new_bin'] = tmp_df.Length.apply(lambda x: this_guy(x, new_bins))

    for a_num, val in enumerate(new_names):
        y_true = tmp_df.query('new_bin==@a_num').loc[:,['FRS','Chelator']].values
        y_pred = tmp_df.query('new_bin==@a_num').loc[:,['Pred_FRS','Pred_Chel']].values
        new_df = pd.concat([new_df, calc_metrics(y_true, y_pred, idx=val)])
