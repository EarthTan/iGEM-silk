...
2. **环境兼容**。本项目主要开发环境为最新版 macOS，实际部署使用 Ubuntu + RTX5880。对可 CUDA 加速的工具：配置 Dockerfile 方便 GPU 环境部署；代码自动检测 GPU 可用性并 fallback 到 CPU；仅 GPU 可运行的服务在 CPU 环境下明确报错。
...
| DSSP | 8103 | PDB/mmCIF 结构 | 二级结构标注；β-sheet 风险过滤 | CPU（Docker） | [—] mkdssp 算法 + CCD 字典 |
...
| **无需模型** | `[—]` | 纯算法或启动时合成训练 | TIPred、SoDoPE、SASA、DSSP |
